...
export function Play() {
  ...
  // Default to paper funds whenever real play can't work yet (not
  // deployed, or no wallet connected) - always switchable either way so
  // the toggle stays useful once Push is live.
  const [mode, setMode] = useState<Mode>(PUSH_DEPLOYED && isConnected ? 'real' : 'paper');
  ...
  const modeToggle = (
    <div className="mb-4 flex justify-center gap-1 rounded-full bg-zinc-100 p-1 text-xs font-medium dark:bg-zinc-800">
      <button
        type="button"
    ...
    return (
      <div>
        ...
        <div className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950 dark:text-amber-100">
          Connect your wallet to stake and climb, or switch to paper funds above to try it out first.
        </div>
      </div>
    );
  }
